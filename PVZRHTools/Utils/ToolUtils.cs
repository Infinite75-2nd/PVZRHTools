...
{
    public static bool ValidateGameDirectory(string path)
    {
        if (!Directory.Exists(path)) return false;
        return File.Exists(Path.Combine(path, Paths.GameName))
               && File.Exists(Path.Combine(path, Paths.GameAssemblyName))
