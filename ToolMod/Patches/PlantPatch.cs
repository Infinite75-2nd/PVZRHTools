...
public static class PlantPatch
{
    ...
    public static bool PreCrashed(Plant __instance, int level, int soundID, Zombie zombie)
    {
        ...
        if (HardPlant || CrushImmunity) return false;
    ...
    public static bool PreDie(Plant __instance)
    {
        ...
        try
        {
            ...
            // 如果植物血量还大于0，不应该死亡
            if (__instance.thePlantHealth > 0) return true; // 正常死亡流程
    ...
    public static void UpdateHealthCache()
    {
        ...
        try
        {
            ...
            foreach (var plant in allPlants)
            {
                ...
                // 只有当植物血量大于0时才更新缓存
                if (plant.thePlantHealth > 0) LastFrameHealth[plantId] = plant.thePlantHealth;
            }
        }
        catch
        {
