...
public class ShootingManagerPatch
{
                ...
                case 3:
                    menu.RegisterOption(
                        ...
                        (UnityAction)(() =>
                        {
                            ...
                            Object.Instantiate(cannon, Board.Instance.transform);
                        }),
                        PlantType.WallNut,
                        (ZombieType)(-1),
                        Quality.iridescent);
    ...
    public static bool PreRegisterCoreBuff(ShootingManager __instance, MultipleChoiceMenu menu)
    {
        ...
        foreach (var plantType in __instance.CurrentPlants)
        {
            ...
            if (totalDamage == 0f)
                totalDamage = 1f;
            var damageShare = __instance.board.damageReporter.GetDamage(plantType) / totalDamage;
            var buffCount = __instance.GetPlantBuffsCount(plantType);
            ...
            foreach (var buff in config.Buffs)
            {
                ...
                var choiceCount = __instance.GetBuffChoiceCount(plantType, buff.Title);
                if (choiceCount >= buff.MaxCount || !buff.CanAppear)
                    continue;

                var isMutation = buff.Title.Contains("质变");
